...
int stage;//stage=0  rd 180 a<44.145
...
double theta(double t){
	return atan((A*sin(om*t)+B*cos(om*t))/(A*cos(om*t)-B*sin(om*t)+1));
}
double tanA(double t){
	double den=A*cos(om*t)-B*sin(om*t)+1;
	return (sin(om*t)-B)/den/den;
}
double tanB(double t){
	double den=A*cos(om*t)-B*sin(om*t)+1;
	return (A+cos(om*t))/den/den;
}
double ax(double t){
	return acc(t)*cos(theta(t))-vx*vy/(6371000+H);
}
double ay(double t){
	return acc(t)*sin(theta(t))+(vT*vT/(6371000+hT)+vx*vx/(6371000+H))/2-g;
}
double f3(double t){
	return -t*ay(t);
}
